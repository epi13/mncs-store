...
| --- | --- | --- |
| P1-001, P1-002, P1-003 | partially resolved | Profile 0.16 expresses bounded create/write/rename/sync and the native publication admission decision. Host code still performs platform file mechanics, directory durability, and the ordinary large-generation publication sequence. |
...
| P2-004, P2-005 | partially resolved | Native publication admission and a granted effectful publication proof exist. The ordinary large-object lifecycle is not yet fully routed through that entrypoint. |
...
on Profile 0.13; the versioned publication module alone requires Profile 0.16;
the repository compatibility ceiling remains 0.18. The missing no-follow
metadata operation is tracked in Commons as `MNCS-LANG-55F049FD8869` and is
not treated as a reason to block the typed Store slice.
